...
import CalculationControl from 'components/CalculationControl';
import { getDbState, dbSave } from 'helpers/localDb';
import {
	isOperator,
	lastCharIsOperator,
	replaceOperator,
...
class Calculator extends React.Component {
	...
	getActionsToCheck = (selectedValue, operatorToDisplay, value) => {
		...
		const mustReplaceOperator =
			isOperator(operators, selectedValue) &&
			lastCharIsOperator(operators, currentCalculation);
		const mustAddOperator =
			isOperator(operators, selectedValue) ||
			lastCharIsOperator(operators, currentCalculation);
	...
	setCalculation = (value, actionsToCheck) => {
		...
		if (notChangeCalculation) {
			...
			if (value !== '.') calculation = `${currentCalculation} ${value}`;
			else calculation = `${currentCalculation} 0${value}`;
	...
	getCalculationToExecute = (selectedValue, operatorToDisplay) => {
		...
		const actionsToCheck = this.getActionsToCheck(
			selectedValue,
			operatorToDisplay,
			value
		);
	...
	handleCalculation = async (selectedValue, operatorToDisplay = '') => {
		if (mustRemoveLastChar(selectedValue)) {
			const { currentCalculation } = this.props;
			const { operators } = this.state;
			const resultAfterRemoveLastChar = removeLastChar(
				currentCalculation,
				operators
			);
		...
		const calculation = this.getCalculationToExecute(
			selectedValue,
			operatorToDisplay
		);
